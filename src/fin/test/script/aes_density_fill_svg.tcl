# Run FIN's original density-fill algorithm on the routed AES design, then
...
read_def "$script_dir/prefill_bench/aes_prefill.def"

if {[info exists ::env(RESULTS_DIR)]} {
  set svg_file "$::env(RESULTS_DIR)/aes_density_fill.svg"
  set density_report "$::env(RESULTS_DIR)/aes_density_fill_density.json"
} else {
  set svg_file "$script_dir/aes_density_fill.svg"
  set density_report "$script_dir/aes_density_fill_density.json"
}
...
if {$metal_area <= 0.0} {
  error "Original density_fill produced no metal area for AES."
}
if { ![file exists "${svg_file}_met1.svg"] } {
  error "Original density_fill did not write the AES placement SVG."
}
