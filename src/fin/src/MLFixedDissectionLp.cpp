...
void validateProblem(const MLFixedDissectionLpProblem& problem)
{
  ...
  if (problem.windows.empty()) {
    throw std::invalid_argument(
        "The multilevel dissection must contain a window.");
  }
    ...
    const MLFixedDissectionLpProblem& problem)
{
  ...
  for (size_t index = 0; index < problem.windows.size(); index++) {
    ...
    // J40 III-B: bound each recorded window's post-fill area.
    ...
    // The objective value is the minimum post-fill area of recorded windows.
  ...
  MLFixedDissectionLpResult result;
  result.solved = true;
